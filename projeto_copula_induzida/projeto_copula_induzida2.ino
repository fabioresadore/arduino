...
void calcularTempoRestante2 (String textinho,long miliseg){
  String min_rest;
  String seg_rest;
  
  int segundos_restantes = 1800 - (miliseg / 1000);
  int minutos_restantes = segundos_restantes / 60;

  int segundos_restantes2 = (miliseg / 1000) - (minutos_restantes * 60);
  int minutos_restantes2 = 30 - minutos_restantes;

  if (minutos_restantes2 < 10){
    min_rest = "0" + String(minutos_restantes2);
  } else {
    min_rest = String(minutos_restantes2);
  }
  
  if (segundos_restantes2 < 10){
    seg_rest = "0" + String(segundos_restantes2);
  } else {
    seg_rest = String(segundos_restantes2);
  }
  
  escreverDisplay(textinho,min_rest + ":" + seg_rest);
}
  ...
  if (state == 2){
    if (ciclo == 1){
      if (millis() > (temp_decorr + 1800000)){
        ...
        calcularTempoRestante2("Baixar temp.",millis() - temp_decorr);
      }
    }
    else if (ciclo == 2){
